...
void radGenDayMtx(int month, int day, float hour, float direct, float diffuse, float albedo, float latitude, float longitude, float standardMeridian, int skyMF, bool sunOnly, bool sharpSun, float rotation, ColorMatrix * skyVec)
{
    ...
    // Calc gendaymtx
    std::string command = "gendaymtx -h -m " + std::to_string(skyMF) + " -g " + std::to_string(albedo) + " " + std::to_string(albedo) + " " + std::to_string(albedo) + " -r " + std::to_string(rotation) + dMode + sharpSunMode + " " + weafilename;
...
TEST(GenPerezSkyVec, FullSky)
{
    ...
    for(int i=0;i<1;i++){
        ...
        // Calculate
        genPerezSkyVector(month, day, hour, direct, difuse, albedo, latitude, longitude, meridian, mf, false, false, 0, &skyVec);
        radGenDayMtx(month, day, hour, direct, difuse, albedo, latitude, longitude, meridian, mf, false, false, 0, &referenceSkyVec);
