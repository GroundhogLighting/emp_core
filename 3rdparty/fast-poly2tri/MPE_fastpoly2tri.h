...
#include <math.h>  // fabs fabsf
  ...
  typedef double f64;
  typedef size_t uxx;
  typedef int bxx;// REPLACED FOR GLARE typedef ssize_t bxx;
...
// ALSO, I REPLACED ALL THE poly_float BY double (just to avoid compilation warnings)
...
#define MPE_POLY2TRI_EPSILON (double)1e-12f
...
uxx MPE_PolyOrient2D(MPEPolyPoint* PointA, MPEPolyPoint* PointB, MPEPolyPoint* PointC)
{
  ...
  double DeltaLeft = (PointA->X - PointC->X) * (PointB->Y - PointC->Y);
  double DeltaRight = (PointA->Y - PointC->Y) * (PointB->X - PointC->X);
  double Value = DeltaLeft - DeltaRight;
...
bxx MPE_PolyInScanArea(MPEPolyPoint* PointA, MPEPolyPoint* PointB, MPEPolyPoint* PointC, MPEPolyPoint* PointD)
{
  MPE_Assert(PointA != PointB && PointA != PointC && PointA != PointD &&
             ...
             PointC != PointD);
  double oadb = (PointA->X - PointB->X) * (PointD->Y - PointB->Y) - (PointD->X - PointB->X) * (PointA->Y - PointB->Y);
  bxx Result;
  ...
  {
    double oadc = (PointA->X - PointC->X) * (PointD->Y - PointC->Y) - (PointD->X - PointC->X) * (PointA->Y - PointC->Y);
...
{
  double AX = PointA->X - Origin->X;
  double AY = PointA->Y - Origin->Y;
  double BX = PointB->X - Origin->X;
  double BY = PointB->Y - Origin->Y;
...
{
  double AX = Node->Point->X;
  double AY = Node->Point->Y;
  double BX = Node->Next->Next->Point->X;
  double BY = Node->Next->Next->Point->Y;
  double DotProduct = AX * BX + AY * BY;
  double Direction = AX * BY - AY * BX;
  bxx Result = DotProduct < 0 || Direction > 0;
  return Result;
...
{
  bxx Result;
  double adx = PointA->X - pd->X;
  double ady = PointA->Y - pd->Y;
  double bdx = PointB->X - pd->X;
  double bdy = PointB->Y - pd->Y;
...
void MPE_PolyTriangulate(MPEPolyContext* PolyContext)
{
  ...
  {
    double Xmax = PolyContext->Points[0]->X;
    double Xmin = PolyContext->Points[0]->X;
    double Ymax = PolyContext->Points[0]->Y;
    double Ymin = PolyContext->Points[0]->Y;
...
#define MPE_INITIAL_TRIANGLE_FACTOR  (double)0.3f
    double Dx = MPE_INITIAL_TRIANGLE_FACTOR * (Xmax - Xmin);
    double Dy = MPE_INITIAL_TRIANGLE_FACTOR * (Ymax - Ymin);
